...
export class SecureCookieManager {
  ...
  public updateSessionActivity(): void {
    ...
    this.setCookie(COOKIES.SESSION_TIMESTAMP, now.toString(), {
      expires: this.createExpiryDateFromDays(
        COOKIE_CONFIG.USER_DATA_EXPIRY_DAYS
      ),
    });
  }
  ...
  public setAuthTokens(accessToken: string, refreshToken: string): void {
    ...
    if (accessValidation.expiresAt) {
      this.setCookie(
        COOKIES.TOKEN_EXPIRY,
        accessValidation.expiresAt.toString(),
        {
          expires: this.createExpiryDateFromDays(
            COOKIE_CONFIG.REFRESH_TOKEN_EXPIRY_DAYS
          ),
    ...
    Object.values(COOKIES).forEach((cookieName) => {
      ...
      if (value) {
        ...
        if (
          ...
          cookieName === COOKIES.REFRESH_TOKEN
        ) {
  ...
  /**
   * Debug cookie accessibility for troubleshooting
   */
  public debugCookieAccess(): { canRead: boolean; details: Record<string, any> } {
    ...
    return {
      ...
      details: {
        ...
        sameSitePolicy: this.isSecureContext() ? 'Lax (secure)' : 'Lax',
        secureContext: this.isSecureContext(),
        accessTokenValid: accessToken ? this.validateToken(accessToken).isValid : false,
        refreshTokenValid: refreshToken ? this.validateToken(refreshToken).isValid : false,
      }
    };
  }
...
export const cookieUtils = {
  ...
  setAccessToken(token: string): void {
    ...
    if (refreshToken) {
      ...
      expires.setTime(
        expires.getTime() +
          COOKIE_CONFIG.ACCESS_TOKEN_EXPIRY_MINUTES * 60 * 1000
      );
      secureCookieManager["setCookie"](COOKIES.ACCESS_TOKEN, token, {
        expires,
      });
    }
  },
  ...
  setRefreshToken(token: string): void {
    ...
    if (accessToken) {
      ...
      expires.setTime(
        expires.getTime() +
          COOKIE_CONFIG.REFRESH_TOKEN_EXPIRY_DAYS * 24 * 60 * 60 * 1000
      );
      secureCookieManager["setCookie"](COOKIES.REFRESH_TOKEN, token, {
        expires,
      });
    }
  },
